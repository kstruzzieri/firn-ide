...
| Milestone 4: Run Profiles | **IN PROGRESS** | #16-17, #59-64 complete; #18/#71 Phase 1 (#123) + #71 P2 panel (#125) + P2 follow-ups/recency sidecar (#127) + #18 P3 header selector (#129) + lifecycle-script detection fix (#130) shipped → #71 closed; #18 P4, #103, #107 open |
...
Current status: **Milestone 4 #18 P3 (header run-profile selector) shipped via PR #129**, plus a detection fix (PR #130) that stops npm install/publish/pack lifecycle hooks (e.g. husky `prepare`) from surfacing as run profiles — they were appearing under a polyglot repo root that classifies as a Go workspace. The header is a split control naming one **selected** profile — the single **Cmd+R** target — with a dropdown grouped by workspace → section (inline run/stop/restart + env variant). Selection is shared with the four-section panel (same target marker), session-only with a recency-based default resolver; no backend or contract change. Remaining Run Profiles UI: **P4** create/edit form (#18). Two manual gates were tracked in #129 (verify Cmd+R does not reload a packaged macOS build — native menu fallback ready if it does — and an a11y keyboard smoke).
...
1. **#18: Run Profiles UI (Phase 4)** — Phases 1–3 shipped (#123 detection/identity, #125 P2 panel, #129 P3 header selector + Cmd+R); profiles carry an owning workspace, the panel is the four-section working set, and the header has the selector dropdown + single Cmd+R run target. Remaining:
