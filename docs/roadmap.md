...
| Future Features | **IN PROGRESS** | #44 implementation shipped via PR #206 (tracker still open); #45/#46 are unlocked; #226 Golem chat panel shipped via PR #262 on the #165 embedded runtime; #263 slices A and B shipped via PRs #269/#270/#272, Slice C named profiles and the narrow-pane redesign via PR #333 (closing #284/#308/#309/#310/#315), and exposure chips, filled model cards and the model note via PR #343 (2026-09-20); #285 phase routing shipped via PR #287, and #271 center panel and undock shipped via PR #288; #264/#265, the polish follow-ups #289/#291/#293, and the configuration-workspace follow-ups filed from its live gates and reviews (#307, #311, #312, #314, #316-#323, #334-#342, #344, #345) remain open |
...
2. **#263 — Golem configuration workspace, via PRs #269/#270/#272/#333/#343:**
   Phase 1 read-only projection and diagnostics, Slice A complete projection
   with typed diagnostics, Slice B transactional writes, Slice C named
   configuration profiles with the narrow-pane redesign (grouped source
   picker, subgrid tables, reach-marked rows, staged-change bar), and the
   exposure chips, filled model cards and model note on the route editor.
   The add-use-case control and Slice D (inventory: a Refresh list from what
   Golem detects, a Check on an unverified capability) remain.
...
| Partly shipped | #263 Golem settings UI | Models, roles, and keys from the panel; project keys write-only so a stored secret never reads back. Phase 1 merged (PR #269); Slice A read-only diagnostics merged (PR #270); Slice B write phases merged (PR #272); Slice C named configuration profiles merged with the narrow-pane redesign (PR #333); exposure chips, filled model cards and the model note merged (PR #343). The add-use-case control and Slice D (inventory: Refresh list, Check) remain — Slice D no longer owns the go-llm pin bump, which the #285 phase-routing/destination-admission consumer slice satisfies. |
...
#263 slices A, B and C added the configuration workspace, its named profiles,
the narrow-pane redesign and the route editor's exposure chips and model
cards (PRs #269/#270/#272/#333/#343), #285 added phase routing and
destination admission, and #271 moved the chat into a center island that
optionally undocks into a second native window.

Remaining: #263's add-use-case control and Slice D inventory, #264 durable
