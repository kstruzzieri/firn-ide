...
6. **#146 Phases 2A-2C:** Phase 2A merged via PR #224, Phase 2B same-profile parallelism shipped via PR #232, and Phase 2C persisted run history is delivered by the current #146 pull request. `internal/runhistory` retains 50 summaries and 5 rich ordinary records per profile, with 10 MiB per-record and 20 MiB per-workspace bounds. Terminal events enqueue persistence; close gives appends a best-effort 300 ms drain, waits durably for explicit redactions, and still withholds acknowledgement when editor flush fails. Only Phase 2D's execution-plan abstraction remains deferred.

PR #227 additionally fixed a clipped adopt button in the run-profile card action row.

### Parallel development plan

#### Current wave — four isolated worktrees
...
Three design notes from Phase 3 worth preserving for Phase 4: session identity is `requestRevision`, not the workspace-scoped `epoch`; preview is deliberately DOM-only rather than a `StateEffect`, because any dispatch rebuilds the card and destroys keyboard focus; and the decorations builder declares `['doc', field]` explicitly so a future field memoization cannot silently stale the provenance stripes.
